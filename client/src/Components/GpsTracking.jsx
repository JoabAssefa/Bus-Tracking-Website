...
const busConfigs = {
  Bus1: { channelId: "2545299", apiKey: "C9Q9Y9SR9WCTGQFH" },
  Bus2: { channelId: "2554524", apiKey: "LY0LHQ2N8WEN5LQY" },
  Bus3: { channelId: "2554526", apiKey: "Q89M9ONZ57Y0HMMM" },
  Bus4: { channelId: "2554527", apiKey: "02M7ZVLZDJJX3KX1" },
};
...
const GpsTracking = () => {
  ...
  const fetchData = () => {
    const { channelId, apiKey } = busConfigs[selectedBus];
    fetch(
      `https://api.thingspeak.com/channels/${channelId}/feeds/last.json?timezone=Asia%2FJakarta&api_key=${apiKey}`
    )
  ...
  const updateMarker = (speed, latitude, longitude, dateNow, timeNow) => {
    ...
    // Add an indicator for tracking status
    if (statBtn === 1) {
  ...
  const startPolyline = () => {
    if (statBtn === 0) {
      ...
      setStatBtn(0);
      polyline.setLatLngs([]);
      mapRef.current.removeLayer(updatedMarkerRef.current);
      mapRef.current.removeLayer(finishMarkerRef.current);
    }
  };
            ...
            <ul className="navbar-nav ml-auto">
              <li className="nav-item dropdown">
                <DropdownButton
                  ...
                  align="end"
                >
                  <NavDropdown.ItemText className="dropdown-header-custom">
                    Bus List
                  </NavDropdown.ItemText>
                  <NavDropdown.Item
                    className="dropdown-item-custom"
                    onClick={() => setSelectedBus("Bus1")}
                  >
                    - Bus 1
                  </NavDropdown.Item>
                  <NavDropdown.Item
                    className="dropdown-item-custom"
                    onClick={() => setSelectedBus("Bus2")}
                  >
                    - Bus 2
                  </NavDropdown.Item>
                  <NavDropdown.Item
                    className="dropdown-item-custom"
                    onClick={() => setSelectedBus("Bus3")}
                  >
                    - Bus 3
                  </NavDropdown.Item>
                  <NavDropdown.Item
                    className="dropdown-item-custom"
                    onClick={() => setSelectedBus("Bus4")}
                  >
                    - Bus 4
                  </NavDropdown.Item>
                  <Dropdown.Divider />
                  <Dropdown.Item
                    className="dropdown-item-records"
                    as={Link}
                    to="/records"
                  >
                    Records
